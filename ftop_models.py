...
def cv_model(model, h_params, X_train, y_train):
    model_cv = GridSearchCV(estimator=model, param_grid=h_params, scoring='neg_root_mean_squared_error', return_train_score=True)
...
#test_scores = lasso_raw.cv_results_['mean_test_score']
#train_scores = lasso_raw.cv_results_['mean_train_score']
...
sns.barplot(x = ['Lasso-raw', 'Lasso-std', 'KNN-raw', 'KNN-std', 'MLP-raw', 'MLP-std'], y = [t1, t2, t4, t5, t7, t8])
plt.ylabel('seconds')
...
print_metrics('LASSO RAW', y_test, y_pred_lasso_raw, lasso_raw_train_time)
...
print_metrics('LASSO PRE-PROCESSED', y_test, y_pred_lasso_std, lasso_std_train_time)
...
print_metrics('LASSO ENSEMBLE PRE-PROCESSED', y_test, y_pred_lasso_ensemble, lasso_ensemble_train_time)
...
print_metrics('KNN RAW', y_test, y_pred_knn_raw, knn_raw_train_time)
...
print_metrics('KNN PRE-PROCESSED', y_test, y_pred_knn_std, knn_std_train_time)
...
print_metrics('KNN ENSEMBLE RAW', y_test, y_pred_knn_ensemble, knn_ensemble_train_time)
...
print_metrics('MLP RAW', y_test, y_pred_mlp_raw, mlp_raw_train_time)
...
print_metrics('MLP PRE-PROCESSED', y_test, y_pred_mlp_std, mlp_std_train_time)
...
print_metrics('MLP ENSEMBLE PRE-PROCESSED', y_test, y_pred_mlp_ensemble, mlp_ensemble_train_time)
